...
extension Array where Element == Device {
    func deduplicated() -> [Device] {
        let dict = Dictionary(self.map { ($0.uniqueDeviceId, $0) }) { (current, new) in
            if new.connectionType == "USB" {
                return new
            } else {
                return current
            }
...
extension Device {
    public var localizedModel: LocalizedStringKey {
        switch productType {
        ...
        default: return LocalizedStringKey(productType)
        }
    }
